...
class LanguageLocalDataSourceImpl implements LanguageLocalDataSource {
  ...
  Future cacheLanguage(String language) async {
    try {
      bool exists = await localStorageService.checkIfExists(HiveConstants.language);
      if (exists) {
  ...
  Future<String> getCachedLanguage() async {
    try {
      String cachedLanguage =
          await localStorageService.get(HiveConstants.language,key: HiveConstants.language) ?? '';
